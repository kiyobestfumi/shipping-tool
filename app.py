...
# --- 【最新版】場所マスターデータの定義 ---
# 住所（address）の項目を追加しました。ここを書き換えるだけで登録・変更が可能です。
LOCATION_MASTER = {
    ...
    if st.button("データを抽出してExcelとテキストを生成する"):
        with st.spinner("最新のGemini AIと通信してデータを抽出しています...（約10秒）"):
            try:
                ...
                # ★追加ポイント：顧客送付用の情報（RECEIPT, CFS CUT, LOADING, DELIVERY）を抽出項目に追加
...
情報が見当たらない場合は空文字("")にしてください。
{
  ...
  "shipper": "荷主名",
  "quantity": "外装個数（※必ず左枠の数値と、そのすぐ隣にある単位（CS, PLT, 種類等）を組み合わせて '3 CS' や '1 PLT' のように抽出してください。右枠にあるバラの数字や重量用の数字は絶対に使用しないでください）",
  "weight": "重量 (数字のみ)",
  "volume": "体積 M3 (数字のみ)",
...
※貨物送り状には、ブッキングナンバーを明記し、  「ベストシッピング扱い」とご記載ください。"""
                ...
                # st.code を使うと、右上に便利な「コピーボタン」が付きます
                st.code(email_text, language="text")
                
                # (念のためJSONの元データも折りたたみで確認できるようにしておきます)
